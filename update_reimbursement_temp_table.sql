-- UPDATE USING TEMPORARY TABLE (If IN clause is too long)
-- Step 1: Create temp table with invoice numbers
CREATE TEMPORARY TABLE temp_invoice_list (
    invoice_no VARCHAR(50) PRIMARY KEY
);

-- Step 2: Insert invoice numbers
INSERT INTO temp_invoice_list (invoice_no) VALUES
...
-- Step 3: Update using temp table
...
INNER JOIN (
    ...
    INNER JOIN temp_invoice_list til ON lcim.invoice_no = til.invoice_no
    WHERE tfd.status <> 99
...
SELECT 
    ...
    (SELECT SUM(COALESCE(reimbursement_amount, 0)) 
     FROM transfer_fee_details 
     WHERE loan_case_invoice_main_id = im.id AND status <> 99) as calculated_amt,
    (SELECT SUM(COALESCE(reimbursement_sst_amount, 0)) 
     FROM transfer_fee_details 
     WHERE loan_case_invoice_main_id = im.id AND status <> 99) as calculated_sst_amt
FROM loan_case_invoice_main im
INNER JOIN temp_invoice_list til ON im.invoice_no = til.invoice_no
ORDER BY im.invoice_no;

-- Step 5: Clean up (optional - temp table auto-drops on connection close)
DROP TEMPORARY TABLE IF EXISTS temp_invoice_list;
