...
@section('content')
    ...
    <div class="container-fluid">
        <div class="fade-in">
            ...
            @if (!in_array($current_user->menuroles, ['receptionist']) && !in_array($current_user->id, [0]))
            ...
            <div class="row">
                ...
                @if (in_array($current_user->menuroles, ['admin', 'management'])  || in_array($current_user->id, [37]))
                    <div class="col-xl-6 col-md-12 col-sm-12">
                        <div class="card">
                            <div class="card-header">
                                <strong>Cases by branch</strong>
                            </div>
                            <div class="card-body">
                ...
                @if ((in_array($current_user->menuroles, ['admin', 'management', 'sales']) ||
                        in_array($current_user->id, [37,14, 80, 51, 32, 89])) && !in_array($current_user->id, [0]))
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        async function reloadDashBoardCaseCount() {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    // Get monthly totals for x-axis labels
                    var monthlyTotals = result.data.find(function(item) {
                        return item.branch === 'Monthly Total';
                    });
                    ...
                    if (chartAllCases == null) {
                                    ...
                                    xAxes: [{
                                        ticks: {
                                            callback: function(value, index, values) {
                                                // Return the label with monthly total
                        ...
                        // Update x-axis labels with monthly totals
                        var xLabelsWithTotals = [];
                        for (var i = 0; i < xValues.length; i++) {
                            var monthLabel = xValues[i];
                            if (monthlyTotals && monthlyTotals.count[i] !== undefined) {
        ...
        async function reloadDashBoardCaseCountByBranch() {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    var colors = ['blue', 'green', 'purple', 'red', 'yellow', 'orange', 'brown', 'pink', 'gray', 'cyan', 'magenta', 'lime'];
                    
                    // Filter out the 'Monthly Total' entry and create datasets
                    var branchData = result.data.filter(function(item) {
                        return item.branch !== 'Monthly Total';
                    });
                    ...
                    if (chartAllCasesbyBranch == null) {
                        // Create datasets dynamically based on available branches
                        var datasets = [];
                        for (var i = 0; i < branchData.length; i++) {
                            datasets.push({
                                data: branchData[i].count,
                                label: branchData[i].branch,
                        ...
                        for (var i = 0; i < branchData.length; i++) {
                            if (chartAllCasesbyBranch.data.datasets[i]) {
                                chartAllCasesbyBranch.data.datasets[i].data = branchData[i].count;
                                chartAllCasesbyBranch.data.datasets[i].label = branchData[i].branch;
                            }
                        }
        ...
        async function reloadDashBoardCaseCountBySales() {
            ...
            form_data.append("branch", $("#ddl_branch_sales").val());
