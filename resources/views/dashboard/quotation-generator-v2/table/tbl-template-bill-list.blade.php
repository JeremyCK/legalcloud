...
@if (count($quotation))
    ...
    @foreach ($quotation as $index => $cat)
                ...
                <td class="text-right">
                    @if ($cat['category']->id == 1 || $cat['category']->id == 4)
                        <span id="sst_{{ $details->id }}">
                            {{-- {{ number_format((float) ($formula_amt * 0.06), 2, '.', ',') }}</span> --}}
                            {{ number_format((float) ($formula_amt * $sst_percentage), 2, '.', ',') }}</span>
                    @else
                        -
                    @endif
                </td>
        ...
        @if(count($cat['account_details']) > 0)
         <tr>
            <th colspan="@if ($cat['category']->id == 1)5 @else 6 @endif">Sub Total </th>
            @if ($cat['category']->id == 1)
            <th style="text-align:right" id="sst_total_{{ $cat['category']->id }}" > {{ number_format($total_sst, 2, '.', ',') }}</th>
            @endif
            <th style="text-align:right" id="sub_total_{{ $cat['category']->id }}" >
