                ...
                <div class="col-sm-12">
                    ...
                    <div id="dList" class="card">
                        <div class="card-header">
                            <div class="row">
                                <div class="col-sm-6 col-md-10 col-lg-8 col-xl-6 ">
                                    <h4>Edit E-Invoice Batch Records</h4>
                                </div>
                        ...
                        <div class="card-body">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input class="form-control" value="{{ $EInvoiceMain->ref_no }}"
                                                    name="ref_no" id="ref_no" type="text" readonly />
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="col-md-8">
                                                <select class="form-control" name="batch_status" id="batch_status">
                                                    <option value="NOTSENT"
                                                        @if ($EInvoiceMain->batch_status == 'NOTSENT') selected @endif>Pending</option>
                                                    <option value="SQL"
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" id="pfee_only" name="pfee_only" value="1" @if(isset($EInvoiceMain->pfee_only) && $EInvoiceMain->pfee_only) checked @endif>
                                                    <label class="form-check-label" for="pfee_only">PFee Only</label>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {{-- Display download statuses --}}
                            <div class="row mt-3">
                                <div class="col-sm-12">
                                    <p><strong>Invoice Template:</strong> {{ $EInvoiceMain->invoice_download_status ?? "Not downloaded yet" }}</p>
                                    <p><strong>Customer Templates:</strong> {{ $EInvoiceMain->client_download_status ?? "Not downloaded yet" }}</p>
                                </div>
                            </div>
                                ...
                                <div class="btn-group normal-edit-mode">
                                    ...
                                    <div class="dropdown-menu" style="padding: 0px; margin: 0px;">
                                        <a class="dropdown-item btn-success" href="javascript:void(0)"
                                            onclick="generateSQLExcelTemplate({{ $EInvoiceMain->id }});" style="color:white;margin:0"><i
                                                style="margin-right: 10px;" class="fa fa-file-excel-o"></i>Generate SQL Invoice Template</a>
                                        <div class="dropdown-divider" style="margin:0"></div>
                                        <a class="dropdown-item btn-success" href="javascript:void(0)"
                                            onclick="generateSQLCustomerTemplate({{ $EInvoiceMain->id }});" style="color:white;margin:0"><i
                                                style="margin-right: 10px;" class="fa fa-file-excel-o"></i>Generate SQL Customer Template</a>
                                        <div class="dropdown-divider" style="margin:0"></div>
                                        <a class="dropdown-item btn-warning" href="javascript:void(0)"
                                            onclick="sendToSQLAccounting();"
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12">
                            ...
                            <a class="btn btn-lg btn-info  float-right" href="javascript:void(0)"
                                onclick="reloadTable();">
                                <i class="fa cil-search"> </i>Searcha
                            </a>
                        </div>
...
@section('javascript')
    ...
    <script>
        ...
        function generateSQLExcelTemplate(id) {
            ...
            $.ajax({
                type: 'GET',
                url: '/generate-sql-excel-template/' + id,
                success: function(response) {
                    ...
                    if (response.status === 1) {
                        ...
                        toastController('Excel template generated successfully');
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: response.message || 'Error generating Excel template',
                ...
                error: function(xhr, status, error) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Error generating Excel template: ' + error,
                        confirmButtonText: 'OK'
                    });
                }
            });
        ...
        function generateSQLCustomerTemplate(id) {
            // Show loading indicator
            Swal.fire({
                title: 'Generating Excel...',
                text: 'Please wait while the Excel file is being generated.',
            ...
            $.ajax({
                type: 'GET',
                url: '/generate-sql-customer-template/' + id,
                success: function(response) {
                    // Close loading indicator
                    Swal.close();
        ...
        function sendToSQLAccounting() {
            ...
            if (selectedInvoices.length === 0) {
                Swal.fire({
                    ...
                    text: 'Please select at least one invoice to send to SQL Server.',
                    confirmButtonText: 'OK'
                });
                return;
            }
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "Do you want to send the selected invoices to SQL Server?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    form_data.append("invoice_ids", JSON.stringify(selectedInvoices));
                    form_data.append("einvoice_main_id", {{ $EInvoiceMain->id }});

                    $.ajax({
                        type: 'POST',
                        url: '/einvoice/send-to-sql', // We will define this route next
                        data: form_data,
                        processData: false,
                        contentType: false,
                        success: function(response) {
                            if (response.status === 1) {
                                Swal.fire(
                                    'Sent!',
                                    'Selected invoices have been sent to SQL Server.',
                                    'success'
                                );
                                // Optionally reload the table or update rows visually
                                reloadAddTable();
                            } else {
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Error',
                                    text: response.message || 'Error sending invoices to SQL Server.',
                                });
                            }
                        },
                        error: function(xhr, status, error) {
                            Swal.fire({
                                icon: 'error',
                                title: 'AJAX Error',
                                text: 'An error occurred while sending data: ' + error,
                            });
                        }
                    });
                }
        ...
        function sendToLHDN() {
            ...
            if (selectedInvoices.length === 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'No Invoice Selected',
                    text: 'Please select at least one invoice to send to LHDN.',
                    confirmButtonText: 'OK'
                });
                return;
            }
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "Do you want to send the selected invoices to LHDN?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '/einvoice/send-to-lhdn', // We will define this route next
                        data: form_data,
                        processData: false,
                        contentType: false,
                        success: function(response) {
                            if (response.status === 1) {
                                Swal.fire(
                                    'Sent!',
                                    'Selected invoices have been sent to LHDN.',
                                    'success'
                                );
                                // Optionally reload the table or update rows visually
                                reloadAddTable();
                                ...
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Error',
                                    text: response.message || 'Error sending invoices to LHDN.',
                                });
                            }
                        },
                        error: function(xhr, status, error) {
        ...
        function deleteEinvoiceListSelected() {
            ...
            var voucher = {};
            var errorCount = 0;
            var restrictedDeletion = false;
            var restrictedMessages = [];

            $.each($("input[name='add_bill']:checked"), function() {
                itemID = $(this).val();
                var status = $(this).data('status');

                if (status === 'LHDN' || status === 'SQL') {
                    restrictedDeletion = true;
                    restrictedMessages.push('Invoice ID ' + itemID + ' (Status: ' + status + ')');
                } else {
                    voucher = {
                        id: itemID
                    };
            ...
            if (restrictedDeletion) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Deletion Restricted',
                    html: 'The following invoices cannot be deleted as they have already been sent to LHDN or SQL:<br>' + restrictedMessages.join('<br>'),
                    confirmButtonText: 'OK'
                });
                return;
            }
        ...
        function UpdateEinvoice() {
            var formData = $('#form_einvoice').serialize();
            ...
            $.ajax({
                type: 'PUT',
                url: '/einvoice/{{ $EInvoiceMain->id }}', // We will define this route next
                data: formData,
                success: function(response) {
                    if (response.status === 1) {
                        Swal.fire(
                            'Success!',
                            'E-Invoice record updated successfully.',
                            'success'
                        );
                        // Optionally reload the page or update elements
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: response.message || 'Error updating E-Invoice record.',
                        });
                    }
                },
                error: function(xhr, status, error) {
                    Swal.fire({
                        icon: 'error',
                        title: 'AJAX Error',
                        text: 'An error occurred while updating the record: ' + error,
                    });
                }
            });
        }
