...
<head>
    <meta charset="utf-8">
    <title>Proforma_Invoice_{{ $case->case_ref_no }}</title>
    <style>
        @page {
            margin: 8mm;
            size: A4 portrait;
    ...
    @foreach ($pieces as $index => $row)
        ...
        <table class="header-table">
            <tr>
                ...
                <td class="col-center">
                    <h2 style="margin: 0; font-size: 16px; font-weight: bold;">Pro forma Invoice</h2>
                </td>
                <td class="col-right">
                    <b>Date:</b> {{ date('d-m-Y', strtotime($LoanCaseBillMain->created_at)) }}
                </td>
            </tr>
        </table>
                ...
                <td class="col-left">
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-value">
                            <strong class="text-blue">{{ $LoanCaseBillMain->bill_to }}</strong>
                        </span>
                    </div>
                    ...
                    @if (isset($LoanCaseBillMain->bill_to_tax_no))
                        <div class="info-row">
                            ...
                            <span class="info-value">{{ $LoanCaseBillMain->bill_to_tax_no }}</span>
                        </div>
                    @endif
                    ...
                    @if (isset($LoanCaseBillMain->bill_to_address))
                        <div class="info-row">
                            ...
                            <span class="info-value">{!! nl2br(htmlspecialchars($LoanCaseBillMain->bill_to_address)) !!}</span>
                        </div>
                    @endif
                </td>
                <td class="col-right">
                    <!-- Right Column: Case Ref No, Proforma Invoice No -->
                    <div class="info-row">
                    ...
                    <div class="info-row">
                        <span class="info-label-right">Proforma Invoice No:</span>
                        <span class="info-value-right">{{ $LoanCaseBillMain->bill_no }}</span>
                    </div>
                    
                    @if(isset($purchaser_financier_ref_no) && $purchaser_financier_ref_no && $purchaser_financier_ref_no->value)
                        <div class="info-row">
                            <span class="info-label-right">Ref No:</span>
                    ...
                    @foreach ($rowItem as $index2 => $row)
                        ...
                        @elseif($row['row'] == 'item')
                            @php
                                ...
                                // Proforma invoice uses quo_amount_no_sst instead of amount
                                $quo_amount = isset($details->quo_amount_no_sst) ? $details->quo_amount_no_sst : 0;
                                
                                if ($row['category']->taxable == '1') {
                                    $sst_calculation = $quo_amount * $sst_rate;
                                    ...
                                    $totalSST += $row_sst;
                                    $pf_total += $quo_amount;
                                    $total_sub_main += $quo_amount + $row_sst;
                                    $total_amount_main += $quo_amount + $row_sst;
                                } else {
                                    $stamP_duties_count += 1;
                                    $total_sub_main += $quo_amount;
                                    $total_amount_main += $quo_amount;
                                }

                                $subtotal += $row_sst;
                                $row_total = $quo_amount + $row_sst;
                                $row_count += 1;
                            @endphp
                            <tr>
                                <td class="col-desc">
                                ...
                                <td class="col-amount">{{ number_format((float) $quo_amount, 2, '.', ',') }}</td>
                                <td class="col-sst">
                                    @if ($row['category']->taxable == '1')
