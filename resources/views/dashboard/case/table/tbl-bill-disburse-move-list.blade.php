    ...
    @if (count($bill_disburse))
        @foreach ($bill_disburse as $index => $disb)
            ...
            <tr>
                ...
                <td class="text_voucher_all text_voucher_{{ $disb->id }}">{{ $disb->account_name }}</td>
                <td>{{ $disb->remark }}</td>
                ...
                <td class="text-center remove-this">
                    ...
                    <div class="checkbox">
                        <input type="checkbox" name="move_id" onchange="manageVoucherDetails({{ $disb->voucher_main_id }},{{ $disb->id }})" class="move_all check_voucher_{{ $disb->voucher_main_id }}" value="{{ $disb->id }}" id="chk_{{ $disb->id }}" >
                        <label for="chk_{{ $disb->id }}"> </label>
                    </div>
                    ...
                    {{-- <div class="btn-group">
                        ...
                        <div class="dropdown-menu">
                            <a class="dropdown-item" target="_blank" href="/voucher/{{ $disb->voucher_id }}/edit"><i
                                    class="cil-pencil"></i>Edit</a>
                            @if ($disb->account_approval != 1)
                                <div class="dropdown-divider"></div>
                                <a class="dropdown-item" href="javascript:void(0)"
                                    onclick="deleteVoucher('{{ $disb->voucher_id }}')"><i class="cil-x"></i>Delete</a>
                            @else
