...
$totalNoSST = 0;
$sstTotal = 0;
$grandSST = 0; // NEW: accumulate SST across categories
...
@if (count($quotation))
    ...
    @foreach ($quotation as $index => $cat)
        ...
        {{-- @if(count($cat['account_details']) > 0) --}}
            <tr style="background-color:grey;color:white">
                ...
                <?php
                // reset per-category totals only when the category actually has items
                $subtotal = 0;
                $subtotalnosset = 0;
                $subtotalOri = 0;
                $subtotalEdit = 0;
            ...
            <tr>
                    ...
                    @if (in_array($current_user->menuroles, ['account', 'admin', 'management', 'sales', 'clerk', 'lawyer', 'chambering', 'maker']))  
                        @if ($details->pfee1_item != 1 || in_array($current_user->menuroles,['admin','account','maker']))
                            <a href="javascript:void(0)" data-backdrop="static" data-keyboard="false"
                                ...
                                data-toggle="modal" data-target="#myModal" class="btn btn-xs btn-primary"><i class="cil-pencil"></i></a>
                        @endif
                    @endif
                @else
                ...
                @if (in_array($current_user->menuroles, ['account', 'admin', 'management', 'sales', 'clerk', 'lawyer', 'chambering', 'maker']))
                    <td class="text-right" id="amt_sst_{{ $details->id }}">
                        ...
                        <?php
                        if ($cat['category']->id == 1 || $cat['category']->id == 4) {
                            // accumulate per-item SST (rounded) for the current category
                            $sstTotal += round($details->quo_amount_no_sst * $sst_rate, 2);
                        }
                        ?>
                    </td> {{-- IMPORTANT: close this td before opening the next one --}}
        ...
        {{-- ADD totals ONLY if this category has items --}}
        @if(count($cat['account_details']) > 0)
            <?php
            $total      += $subtotal;
            ...
            $grandSST   += $sstTotal; // accumulate category SST to grand total
            ?>
    ...
    <tr>
        ...
        @if (in_array($current_user->menuroles, ['account', 'admin', 'management', 'sales', 'clerk', 'lawyer', 'chambering', 'maker']))
            <td style="text-align:right" class=""> RM {{ number_format((float) $totalNoSST, 2, '.', ',') }}</td>
            <td style="text-align:right" class="" id="total_edit"> RM
                {{ number_format((float) $grandSST, 2, '.', ',') }}</td>
            <td style="text-align:right" class="" id="total_edit"> RM
                {{ number_format($totalNoSST + $grandSST, 2, '.', ',') }}</td>
        @endif
    </tr>
@else
    <tr>
