...
class EInvoiceContollerV2 extends Controller
{
    ...
    public function generateSQLExcelTemplate($id)
    {
        try {
            ...
            // Debug: Check if SST is being retrieved for the specific record
            foreach ($einvoiceDetails as $debugDetail) {
                if (abs($debugDetail->amount - 204.81) < 0.01) {
                    \Log::info("Excel Export - Query Result Debug for 204.81", [
                        'amount' => $debugDetail->amount,
                        'sst_value' => $debugDetail->sst ?? 'NOT SET',
                        'sst_type' => gettype($debugDetail->sst ?? null),
                        'account_name' => $debugDetail->account_name ?? 'N/A',
                        'all_properties' => array_keys((array)$debugDetail)
                    ]);
                }
            }
            ...
            foreach ($einvoiceDetails as $detail) {
                ...
                if (!in_array($categoryKey, $processedCategories)) {
                    ...
                    if ($detail->account_cat_id == 1) {
                        ...
                        // Check if SST value exists and is valid (matching invoice print template logic)
                        $hasCustomSst = false;
                        if ($sstColumnExists && isset($detail->sst) && $detail->sst !== null && trim((string)$detail->sst) !== '') {
                            ...
                            $taxAmt = (float) $detail->sst;
                            $hasCustomSst = true;
                        }
                        
                        if (!$hasCustomSst) {
                            // Fallback: Calculate SST
                            $taxAmt = ($detail->sst_rate/100) * $detail->amount;
                        }
                    }
                    else if ($detail->account_cat_id == 4) {
                        // Reimbursement category - also has SST
                        $account_code="500-010";
                        $sv="SV";
                        // Use SST from database if available (same as invoice details page), otherwise calculate
                        if ($sstColumnExists && isset($detail->sst) && $detail->sst !== null && $detail->sst !== '') {
                            // Use the stored SST value from database
                            $taxAmt = floatval($detail->sst);
                        } else {
                            // Fallback: Calculate SST
                            $taxAmt = ($detail->sst_rate/100) * $detail->amount;
                        }
                    ...
                    $sheet->setCellValue('V' . $row, $detail->property_title);
                ...
                if ($detail->account_cat_id == 1) {
                    ...
                    // Debug: Log SST retrieval attempt
                    if (abs($detail->amount - 204.81) < 0.01) {
                        \Log::info("Excel Export - SST Debug for 204.81", [
                            'sstColumnExists' => $sstColumnExists,
                            'sst_value' => $sstValue,
                            'sst_type' => gettype($sstValue),
                            'property_exists' => property_exists($detail, 'sst'),
                            'isset_sst' => isset($detail->sst),
                            'sst_not_null' => $sstValue !== null,
                            'sst_not_empty' => $sstValue !== null ? (trim((string)$sstValue) !== '') : false,
                            'is_numeric' => $sstValue !== null ? is_numeric($sstValue) : false,
                    ...
                    if ($sstValue !== null && trim((string)$sstValue) !== '' && is_numeric($sstValue)) {
                        ...
                        // Debug log
                        if (abs($detail->amount - 204.81) < 0.01) {
                            \Log::info("Excel Export - Using database SST", [
                                'amount' => $detail->amount,
                                'sst_from_db' => $taxAmt,
                                'sst_raw_value' => $sstValue,
                                'account_name' => $detail->account_name ?? 'N/A'
                            ]);
                        }
                    }
                    ...
                    if (!$hasCustomSst) {
                        ...
                        // Debug log
                        if (abs($detail->amount - 204.81) < 0.01) {
                            \Log::info("Excel Export - Calculating SST (database value not found)", [
                                'amount' => $detail->amount,
                                'sst_rate' => $detail->sst_rate ?? 'N/A',
                                'calculated_sst' => $taxAmt,
                                'sst_value_checked' => $sstValue,
                                'sstColumnExists' => $sstColumnExists,
                                'property_exists' => property_exists($detail, 'sst'),
                                'account_name' => $detail->account_name ?? 'N/A'
                            ]);
        ...
        public function generateSQLCustomerTemplate($id)
    {
        try {
            ...
            // Get customer details from invoice_billing_party
            $customerDetails = DB::table('invoice_billing_party AS ibp')
                ->leftJoin('loan_case_invoice_main as im', 'im.bill_party_id', '=', 'ibp.id')
                ->leftJoin('loan_case_invoice_details as qd', 'qd.invoice_main_id', '=', 'im.id')
                ->leftJoin('loan_case_bill_main as b', 'b.id', '=', 'qd.loan_case_main_bill_id')
                ->leftJoin('einvoice_details as ed', function($join) use ($id) {
                    $join->on('ed.loan_case_invoice_id', '=', 'qd.invoice_main_id')
                         ->where('ed.einvoice_main_id', '=', $id)
                         ->where('ed.status', '<>', 99);
                }) // CORRECTED: Join einvoice_details by invoice_id, not bill_id
                ->leftJoin('einvoice_main as em', 'em.id', '=', 'ed.einvoice_main_id')
                ->select(
                    'ibp.customer_code',
                    'ibp.customer_name',
                    'ibp.brn',
    ...
    public function generateSQLExcelTemplateBak($id)
    {
        try {
            ...
            $einvoiceDetails = DB::table('loan_case_invoice_details AS qd')
                ->leftJoin('account_item AS a', 'a.id', '=', 'qd.account_item_id')
                ->join('loan_case_bill_main as b', 'b.id', '=', 'qd.loan_case_main_bill_id') // Join to get invoice_no and invoice_date
                ->join('loan_case as l', 'l.id', '=', 'b.case_id') // Join to get case_ref_no
                ->join('client as c', 'c.id', '=', 'l.customer_id') // Join to get client name
                ->join('einvoice_details as ed', 'ed.loan_case_invoice_id', '=', 'qd.invoice_main_id') // CORRECTED: Join einvoice_details by invoice_id
                ->join('einvoice_main as em', 'em.id', '=', 'ed.einvoice_main_id') // Join einvoice_main
                ->select(
                    'em.ref_no as einvoice_ref_no',
                    'em.einvoice_date as einvoice_main_date',
                    'em.transaction_id as einvoice_transaction_id',
