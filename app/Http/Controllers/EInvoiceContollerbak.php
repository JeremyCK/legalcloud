...
class EInvoiceContoller extends Controller
{
    ...
    public function splitInvoice(Request $request, $bill_main_id)
    {
        ...
        for ($i = 0; $i < count($party_masterlist); $i++) {
            array_push($parties_list,  array('party' =>  $party_masterlist[$i]->master_cat_name, 'name' => $party_masterlist[$i]->value));
             ...
             // Check if a billing party with the same bill_id, case_id, and customer_name already exists
        // $party_count = LoanCaseInvoiceMain::where('loan_case_main_bill_id', $bill_main_id)
        //     ->count();
            
        if(count($parties_list) < 2)
        {
            return response()->json(['status' => 0, 'message' => 'At least 2 billing party is required to split the bill.']);
        }
        ...
        if (!$InvoiceBillingParty) {
            ...
            $InvoiceBillingParty->loan_case_main_bill_id = $bill_main_id;
            $InvoiceBillingParty->invoice_main_id = 0;
            $InvoiceBillingParty->case_id = $LoanCaseBillMain->case_id;
        ...
        if (LoanCaseInvoiceMain::where('bill_party_id', $InvoiceBillingParty->id)->where('loan_case_main_bill_id', $bill_main_id)->count() > 0)
        { 
            return response()->json(['status' => 2, 'message' => 'Current party already added in another invoice']);
        }
        ...
        $InvoiceBillingParty->loan_case_main_bill_id = $bill_main_id;
        $InvoiceBillingParty->invoice_main_id = $invoice_main_id_new;
        $InvoiceBillingParty->save();
    ...
    public function getTransferFeeBillListV2(Request $request)
    {
        if ($request->ajax()) {
            ...
            return response()->json([
                'view' => view('dashboard.transfer-fee.table.tbl-transfered-list', compact('rows'))->render(),
            ]);
        ...
        if ($request->ajax()) {
            ...
            if ($request->input("transaction_type") <> 0) {
                if ($request->input("transaction_type") == 2) {
                    $safe_keeping->whereIn('m.type', ['JOURNAL_OUT', 'TRANSFER_OUT', 'BILL_DISB', 'TRUST_DISB', 'SST_OUT', 'CLOSEFILE_OUT', 'ABORTFILE_OUT']);
                } else if ($request->input("transaction_type") == 1) {
                    $safe_keeping->whereIn('m.type', ['JOURNAL_IN', 'TRANSFER_IN', 'BILL_RECV', 'TRUST_RECV', 'SST_IN', 'CLOSEFILE_IN', 'ABORTFILE_IN']);
                }
            }
            ...
            return DataTables::of($safe_keeping)
                ...
                ->addColumn('transaction_type', function ($data) {
                    ...
                    if (in_array($data->type, ['JOURNAL_OUT', 'TRANSFER_OUT', 'BILL_DISB', 'TRUST_DISB', 'SST_OUT', 'CLOSEFILE_OUT', 'ABORTFILE_OUT'])) {
                        return 'Out';
                    } else {
                        return 'In';
                    }
    ...
    public function getMonthRecon(Request $request)
    {
        ...
        $totalAddCLRDeposit = DB::table('ledger_entries_v2 as m')
            ...
            ->whereMonth('m.date', $d["month"])
            ->whereYear('m.date', $d["year"])
            ->whereIn('m.type', ['BILL_RECV', 'TRUST_RECV', 'JOURNAL_IN', 'TRANSFER_IN', 'SST_IN'])
            ->where('m.is_recon', 1)
            ->where('m.bank_id', '=', $request->input("bank_id"))->sum('amount');
